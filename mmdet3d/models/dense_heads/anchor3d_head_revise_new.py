...
class Anchor3DHeadRevise(BaseModule, AnchorTrainMixin):
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, labels,
        ...
        neg_inds = (labels == bg_class_ind).nonzero(as_tuple=False).reshape(-1)
        ...
        ############################## by haolin ##############################
        # ############## consistent loss by haolin###################

        ## overall ---to --- split
        loss_cls_sum_dim1 = torch.sum(loss_cls, dim=1)
        loss_cls_pos = loss_cls_sum_dim1[pos_inds]
        loss_bbox = torch.sum(loss_bbox,dim=1)
        loss_cls_neg = loss_cls_sum_dim1[neg_inds]

        ### beta compute
        loss_bbox_beta = loss_bbox.detach()
        loss_cls_pos_beta = loss_cls_pos.detach()
        exp_loss_bbox_beta =  torch.exp(-loss_bbox_beta)
        exp_loss_cls_pos_beta = torch.exp(-loss_cls_pos_beta)

        ### formula
        loss_cls_pos_new = loss_cls_pos*exp_loss_bbox_beta + loss_cls_pos
        loss_bbox_new = loss_bbox * exp_loss_cls_pos_beta  + loss_bbox

        ###  split  ---to --- overall
        loss_cls_pos_sum = loss_cls_pos_new.sum()
        loss_cls_neg_sum = loss_cls_neg.sum()
        loss_bbox_sum = loss_bbox_new.sum()

        ### to average loss
        loss_cls = (loss_cls_pos_sum + loss_cls_neg_sum) / num_total_samples
        loss_bbox = loss_bbox_sum / num_total_samples
    ...
    def loss(self,
        ...
        #multiapply will call the loss_single on individual points and return summation of individual point loss as losses_cls
