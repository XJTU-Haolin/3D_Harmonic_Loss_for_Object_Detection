...
class PointRPNHead_Revise(BaseModule):
    ...
    def loss(self,
        ...
        # #########
        # loss_cls= torch.sum(semantic_loss, dim=1)
        # # positive_mask_flatten = torch.flatten(positive_mask,0,-1)
        # loss_bbox = torch.sum(bbox_loss, dim=2)
        # loss_bbox = torch.flatten(loss_bbox,0,-1)
        #
        # ### beta compute
        # loss_bbox_beta = loss_bbox.detach()
        # loss_cls_pos_beta = loss_cls.detach()
        # exp_loss_bbox_beta = torch.exp(-loss_bbox_beta)
        # exp_loss_cls_pos_beta = torch.exp(-loss_cls_pos_beta)
        # ##########
        # loss_cls_pos_new = loss_cls * exp_loss_bbox_beta + loss_cls
        # loss_bbox_new = loss_bbox * exp_loss_cls_pos_beta + loss_bbox
        # # loss_dir_new = loss_dir - ((exp_loss_bbox_beta + exp_loss_cls_pos_beta) / 2) * loss_dir
        # ###  split  ---to --- overall
        # loss_cls_pos_sum = loss_cls_pos_new.sum()
        # # loss_cls_neg_sum = loss_cls_neg.sum()
        # loss_bbox_sum = loss_bbox_new.sum()
        # # loss_dir_sum = loss_dir_new.sum()
        #
        # bbox_loss = loss_bbox_sum
        # semantic_loss = loss_cls_pos_sum / positive_mask.float().sum()
