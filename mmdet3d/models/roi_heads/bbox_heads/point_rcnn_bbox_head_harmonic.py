...
class PointRCNNBboxHead_Revise(BaseModule):
    ...
    def __init__(
            ...
            init_cfg=None):
        super(PointRCNNBboxHead_Revise, self).__init__(init_cfg=init_cfg)
    ...
    def loss(self, cls_score, bbox_pred, rois, labels, bbox_targets,
        ...
        if pos_inds.any() != 0 and self.with_corner_loss:
            ...
            ############### harmonic loss
            loss_bbox = torch.sum(loss_bbox, dim=2)
            loss_bbox = loss_bbox.squeeze()
            loss_cls_pos = loss_cls[pos_inds]
            loss_cls_neg = loss_cls[~pos_inds]
            ...
            ### to average loss
            # loss_dir = loss_cls_pos_sum / num_total_samples
            loss_cls = loss_cls_pos_sum + loss_cls_neg_sum
            loss_bbox = loss_bbox_sum

            losses['loss_cls'] = loss_cls.sum()
